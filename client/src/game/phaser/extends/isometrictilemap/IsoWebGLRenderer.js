...
export default (renderer, src, interpolationPercentage, camera, parentMatrix) => {
    ...
    for (var index = 0; index < list.length; index++)
    {
        ...
        var y = blitterY + bob.y + frame.y - bob.z; // XDX-MOD
        ...
        if (frame.sourceIndex !== prevTextureSourceIndex || frame.assetKey !== prevAssetkey) // XDX-MOD
        {
            pipeline.setTexture2D(frame.glTexture, 0);

            prevTextureSourceIndex = frame.sourceIndex;
            prevAssetkey = frame.assetkey; // XDX-MOD
        }
