...
import Plugin_pouchdb_adapter_node_idb from 'pouchdb-adapter-idb';
...
//============================================================================//
import debuglog from "debug/consolelog"; const clog = debuglog("PrototypeDatabase.js");
//============================================================================//

// ===============================================
// path to server-database (pouchdb or couchdb)
// ===============================================
const _SERVERDATABASEPATH = "http://localhost:3333/db/";

// ===============================================
// prefix = subdirectory for local database
// ===============================================
const _DATABASEPATH_ = ""; // only lowercase allowed

export default class PrototypeDatabase {
  get databaseNameFull() { return _DATABASEPATH_ + this.databaseName; }
  get databaseName() { return this._dbName; }

  get serverDatabase() { return _SERVERDATABASEPATH  + this.databaseName; }
  get remotedb() {
    if (!this._remotedb) this._remotedb = new PouchDB(_SERVERDATABASEPATH + this.databaseName, { skipSetup: true, });
    return this._remotedb; // this.remotedb.allDocs(); // dbStore.basemaps.remotedb.allDocs();.
  }
  get serverdb() { return this.remotedb; }
  ...
  async upsertDocument(newdoc, options) {
    ...
    if (!response) throw new Error("upsertDocument failed");
  ...
  async getAttachmentAsBuffer(docId, attachmentId, options) {
    ...
    //const arraybuffer = await blobToArrayBuffer(blobOrBuffer);
    //var ab = new ArrayBuffer(12);
    //var v = new DataView(ab);

    //clog("XXXX1::", blobOrBuffer, blobOrBuffer instanceof Blob, blobOrBuffer instanceof Buffer, typeof blobOrBuffer);
    //clog("XXXX2::", arraybuffer, arraybuffer instanceof Blob, arraybuffer instanceof ArrayBuffer, typeof arraybuffer);
    //clog("XXXX3::", Buffer.isBuffer(blobOrBuffer), Blob.isBlob(blobOrBuffer));
    return (blobOrBuffer instanceof Blob) ? await blobToArrayBuffer(blobOrBuffer) : blobOrBuffer;
  }
  ...
  // on client: attachement will be saved as blob;
  // on server: attachement will be saved as buffer;
  ...
  */
  ...
  loadPlugins = () => {
    ...
    PouchDB.plugin(Plugin_pouchdb_adapter_node_idb);
