// usage:: import debuglog from "debug/consolelog.mjs"; const clog = debuglog(import.meta.url);
// usage:: import debuglog from "debug/consolelog.js"; const clog = debuglog("testfilename.js");
const debuglog = (_pre) => {
  ...
  const xlog = (mainProp, ...restProps) => {
    ...
    const basename = (_pre) ? _pre + " " : "";
    const formatted_pre = (basename.length>0) ? basename.toString().padEnd(7) : "";
    ...
    */
    const formatted_diffTime = diffTime.toString().padStart(6, " ") // 10000 = 10 sec

    const infoString = `${process.env.CONSOLELOG || ""}${formatted_pre}${nowTime.toLocaleTimeString()}(${formatted_diffTime}):: ${mainProp}`;
    //const infoString = `${process.env.CONSOLELOG || ""}${formatted_pre}${nowTime.toLocaleTimeString()}+${formatted_diffTime} ${mainProp}`;
    console.log(infoString, ...restProps);
  }
  return xlog;
}
